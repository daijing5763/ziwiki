...
import { fetch_pull_repo, fetch_delete_repo } from "../utils/web_fetch"
...
export default function SideBar({ repo_id,layout, SideBarIndex, setSideBarIndex,useSearch,setUseSearch }) {
  ...
  async function syncRepo() {
    const id = toast(t`Update Repo...`, { toastId: "customId", type: toast.TYPE.INFO });
    fetch_pull_repo({ "repo_id": repo_id }, session.access_token).then(data => {
      if (data.error){
        toast.update(id, { render: t`Sync Failed:`   + data.error, type: toast.TYPE.ERROR, isLoading: false});
      } else {
        toast.update(id, { render: t`Sync Succeed`, type: "success", isLoading: false });
      }
    })
  }
  ...
  return (
    <div className="max-w-8xl mx-auto px-4 sm:px-6 md:px-8">
      <DeleteRepo  open={deleteopen} setOpen={setdeleteopen} repo_id={repo_id} />
        ...
        ">
          ...
          <div className="ml-4 mb-8 mt-10 md:mt-0 text-base lg:text-base font-base">
            <Link href="#" onClick={syncRepo} className="group flex items-center lg:text-sm lg:leading-6 mb-4 font-medium text-slate-700 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-300">
              ...
              <Trans>Sync Repo</Trans>
            </Link>
            
            <Link href="#" onClick={()=>setdeleteopen(true)} className="group flex items-center lg:text-sm lg:leading-6 mb-4 font-medium text-slate-700 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-300">
              ...
              </div><Trans>Delete Repo</Trans>
            </Link>

            <Link  href={`/wiki/${repo_id}/home`} className="group flex items-center lg:text-sm lg:leading-6 mb-4 font-medium text-slate-700 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-300">
