...
func (r *TableHTMLRenderer) renderTable(w util.BufWriter, source []byte, n gast.Node, entering bool) (gast.WalkStatus, error) {
	if entering {
		_, _ = w.WriteString(`<div class='relative rounded-xl overflow-auto  flex  justify-center'>
...
func (r *TableHTMLRenderer) renderTableHeader(w util.BufWriter, source []byte, n gast.Node, entering bool) (gast.WalkStatus, error) {
	if entering {
		_, _ = w.WriteString("<thead class='bg-slate-50 dark:bg-slate-700'")
		if n.Attributes() != nil {
			html.RenderAttributes(w, n, TableHeaderAttributeFilter)
		}
		_, _ = w.WriteString(">\n")
		_, _ = w.WriteString("<tr class='flex-1 justify-center items-center'>\n") // Header <tr> has no separate handle
	} else {
...
func (r *TableHTMLRenderer) renderTableCell(w util.BufWriter, source []byte, node gast.Node, entering bool) (gast.WalkStatus, error) {
	...
	tagstart := "td class='border border-slate-300 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400'"
	tagend := "td"
	if n.Parent().Kind() == ast.KindTableHeader {
		tagstart = "th class='border border-slate-300 dark:border-slate-600 font-semibold p-4 text-slate-900 dark:text-slate-200 text-left'"
		tagend = "th"
	}
	if entering {
		fmt.Fprintf(w, "<%s", tagstart)
