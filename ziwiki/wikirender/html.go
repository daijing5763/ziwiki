...
func (r *Renderer) renderList(w util.BufWriter, source []byte, node ast.Node, entering bool) (ast.WalkStatus, error) {
	n := node.(*ast.List)
	tagbegin := "ul class='list-disc  text-base text-slate-800 dark:text-slate-300  pl-4'"
	if n.IsOrdered() {
		tagbegin = "ol class='list-decimal text-base text-slate-800 dark:text-slate-300  pl-4'"
	}
	tagend := "ul"
	if n.IsOrdered() {
		tagend = "ol"
