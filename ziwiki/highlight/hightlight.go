...
func (r *HTMLRenderer) renderFencedCodeBlock(w util.BufWriter, source []byte, node ast.Node, entering bool) (ast.WalkStatus, error) {
	...
	if r.WrapperRenderer != nil {
		...
		<div class="relative flex text-slate-400 text-sm leading-6">
				<div class="mt-2 flex-none dark:text-sky-300 text-slate-600 border-t border-b border-t-transparent border-b-slate-500 dark:border-b-sky-300 px-4 py-1 flex items-center">`)
		language := n.Language(source)
		if language != nil {
