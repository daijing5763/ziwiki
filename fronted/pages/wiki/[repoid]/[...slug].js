...
export default function Home({ session}) {
  ...
  async function getLayout(values) {
    ...
    await fetch('https://localhost/backend/get_markdown', options)
      .then(res => res.json())
      .then((data) => {
        if (data && !data.error) {
    ...
    replace: ({ attribs, children }) => {
      ...
      if (isContains(attribs.class,"mermaid")) {
        ...
        if (slugs && !attribs.src.startsWith('http')) {
          const prefix="https://localhost/backend/static_get/1/"+router.query.repoid+"/"+slugs.slice(0,-1).join("/")+"/"+attribs.src
          return (
